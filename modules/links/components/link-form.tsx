...
import { addSocialLink, createLinkByUser, editSocialLink, updateLink, deleteLink, deleteSocialLink } from "../actions";
...
try {
    ...
    const onEditLinkSubmit = async (data: LinkFormData) => {
        ...
        try {
            const res = await updateLink(data, editingLinkId);
            if (res?.sucess) {
                setLinks((prev) =>
                    prev.map((l) =>
                        l.id === editingLinkId ? { ...l, title: data.title, url: data.url, description: data.description } : l
                    )
                );
    ...
    const handleDeleteSocialLink = async (socialLinkId: string) => {
        try {
            const res = await deleteSocialLink(socialLinkId);
            if (res?.sucess) {
                setUserSocialLinks((prev) => prev.filter((s) => s.id !== socialLinkId));
                toast.success(res.message || "Social link deleted");
            } else {
                toast.error(res?.error || "Failed to delete social link");
            }
        } catch (error) {
                                    ...
                                    <div key={socialLink.id} className="relative group">
                                        ...
                                        >
                                            ...
                                            <Button
                                                size="sm"
                                                variant="ghost"
                                                className="absolute -top-2 -right-8 h-5 w-5 rounded-full p-0 opacity-0 group-hover:opacity-100 transition-opacity"
                                                onClick={() => {
